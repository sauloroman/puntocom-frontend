...
export const ModalConfirmChangeStatusSupplier: React.FC = () => {
  ...
  const { onChangeSupplierStatus, supplierSelected } = useSuppliers()
  ...
  const changeSupplierStatus = () => {
    onChangeSupplierStatus(id!, !isActive)
    onCloseModal()
  }
  ...
  return (
    <ModalLayout width="w-lg">
      <div className="flex flex-col items-center justify-center text-center space-y-4 p-6">
        ...
        <h2 className={`text-xl font-semibold ${isDark ? 'text-white' : 'text-gray-600'}`}>
        ...
        <p className={`${isDark ? 'text-white' : 'text-gray-600'}`}>
          ...
          <span className={`font-medium ${isDark ? 'text-white' : 'text-gray-600'}`}>
            {supplierName}
          </span>?
        </p>
